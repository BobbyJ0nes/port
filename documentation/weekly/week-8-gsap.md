# week 8 · gsap — motion + timeline

> _syllabus topic:_ GSAP animation, timelines, ScrollTrigger, motion as
> narrative.

> _artifact:_ gsap integration inside `/projects/moss/` —
> a short intro timeline that types the terminal lines, fades in the
> product card, and pulses the ADD TO CART button once per cycle.

> ⚠️ **decision / honest note:** the archived `life-of-a-camcorder`
> piece was a candidate for this week but doesn't actually use GSAP
> (it uses IntersectionObserver). rather than ship a piece whose
> framing doesn't match its implementation, i've added a real GSAP
> timeline to `moss` — a piece that already belonged in the portfolio
> and benefits from the motion layer.
...
> - `screens/moss-intro-start.png` — first frame (everything hidden).
> - `screens/moss-intro-end.png` — end frame (everything visible).
> - `clips/moss-intro.mp4` — a 4-second screen capture of the timeline
>   (optional but strongly recommended, so the motion is legible in
>   the submission pdf).
