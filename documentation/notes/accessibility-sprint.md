...
## intent
accessibility as _default craft_, not as a patch. the task was to audit
the portfolio, fix the easy wins, and honestly document what remains.
take one finished project (orion) and one in-progress project (moss) and
move them from "works for me" to "passes axe DevTools with no critical
issues".
...
actual attributes in the html (e.g. "does this skip-link satisfy
